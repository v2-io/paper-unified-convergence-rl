# OUT.full-paper.md — full-paper assembly manifest

*Concatenation order for the full version (no page constraint). Each row is a segment in `src/`; assembly is top-to-bottom. `\appendix` is auto-injected before the first row whose Type is `Appendix`; `\newpage` is auto-injected before any `Checklist` row.*
