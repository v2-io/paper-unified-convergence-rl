...
### Considered and dismissed (with reasoning)

- *Gemini "merge §4 and §5"* and *"relocate ablation §4.4 to appendix"* — both are page-budget thinking. Per Joseph's directive (no compression cycle), skip. Beyond that: merging §4-§5 collapses the Jin-style restructure where §4 states the formal main result and §5 narrates the mechanism with key lemmas surfaced — undoing the restructure's whole point. §4.4 necessity argument is doing reviewer-anticipation work that pays off when reviewers ask "are all four components needed?".
- *Codex trim "NeurIPS Theory Track guideline quote"* — the quote situates the paper in the Theory-Track tradition that values exact analytic identities over empirical fits. AUTHORING §3 calls for this kind of voice positioning; trimming would weaken the paper's self-positioning argument.
- *Codex trim "BH/Pinsker repetition across abstract/intro/§4/§5/§E/§6"* — the repetition is doing genuine emphasis-of-novelty work for a result whose entire technical contribution at this corner is its relation to BH. A tightening pass *might* be worth a single one-shot review post-strategy-talk; until then, hold.
- *Gemini "elevate perturbative extension to main-text Theorem"* — interesting but second-order to H1-H5 / M1 / M3. If we elevate, we'd want the scaffolding (V_max convention, bias term, support condition) cleaned up first so the elevated theorem inherits clean foundations. Revisit post-spikes.
