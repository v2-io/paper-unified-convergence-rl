# B-CS1 Long-Form Paper Draft

*This document is the **uncompressed** version of `02-convergence/paper-draft.md`. Origin: `a724b14:02-convergence/paper-draft.md` (post-Pass-3 body-pass + citation fixes + strengthen-pass + variation-budget formalization, **before** the mechanical-compression and structural-move passes that brought the working draft to 9pp NeurIPS-fit). Use cases: (a) source for journal-version expansion (10–15pp); (b) reference for restoring trimmed content if reviewers ask "where did X go?"; (c) safety net — anything aggressively trimmed from the working draft is preserved here. Future trims should be captured below the source-line by appending to a "Trimmed in <date>" subsection if the cut was substantial.*

> **⚠️ Out-of-sync with `paper-draft.md` after 2026-05-05 Pass-4 audits + spike a5c98b95.** This long-form snapshot predates the Pass-4 audit fixes (M1–M4 math correctness, NC + R-series + B1 calibration, N3/N4 honest narrowings) AND the N1/N2 strengthen-spike (Theorem 7.1(v) trajectory-level lift via simulation lemma; Lemma 5.2 impulsive-system form via Hespanha–Liberzon–Teel 2008). **Single source of truth for current claims is `paper-draft.md`.** Inline pointer notes flag affected sections below. To sync: read `LOG.md` 2026-05-05 entries + `_archive/spike-N1-N2-strengthen-2026-05-05.md` + `_archive/AUDIT-PASS4-{CODEX,CODEX-detailed,GEMINI}.md`, then mechanically apply the listed deltas (each pointer below names the paper-draft section to copy from). The original long-form text is left intact — these are pointer overlays, not in-place rewrites — so the journal-version expansion still has the longer prose available where the short paper had to compress.
...
> **⚠️ SYNC FROM `paper-draft.md` §4.5–4.6 (Pass-4 M1 + M2).** Theorem 4.3 statement and proof in this long-form section are stale: (M1) the softmax correction is $O(\tau^{-1}\exp(-\Delta_{\min}/\tau))$ — the prefactor $1/\tau$ does not get absorbed by the exponential; (M2) the ε-greedy proof here uses an "$Q$ aligned with $\pi^*$" assumption that doesn't generally hold and was replaced in `paper-draft.md` with a uniform perturbation lemma (TV-Lipschitz under point-mass perturbation; reverse-KL uniform expansion under $Q(a) \ge q_0$; map through $1-e^{-x}$ via 1-Lipschitzness; constants depend on $q_0$ and $|\mathcal A|$). To sync: copy Theorem 4.3 statement and the Appendix A.6 derivation from current `paper-draft.md` and replace this subsection's body.
...
> **⚠️ SYNC FROM `paper-draft.md` §5.4 (spike a5c98b95 N2, 2026-05-05).** This Lemma 5.2 statement is **superseded** by the impulsive-system form: under modeled impulsive dynamics with destabilizing continuous evolution between updates and stabilizing impulses with per-update gain $\gamma \in (0,1]$, ultimate boundedness holds iff $\Delta_{\max}\cdot\rho_\Sigma/R_\Sigma < -\ln(1-\gamma)^2$, citing Hespanha–Liberzon–Teel 2008 reverse-ADT. The impulsive form recovers the $K/T$-averaged result under uniform blocks and is **strictly stronger** under nonuniform schedules (longest block sets the threshold, not the average). Surfaces a new design tradeoff between update frequency $1/\Delta_{\max}$ and per-update impulse strength $\gamma$. To sync: copy current Lemma 5.2 (impulsive-system form) from `paper-draft.md` §5.4 verbatim and update the surrounding paragraph. Add HLT 2008 to references. Full derivation in `_archive/spike-N1-N2-strengthen-2026-05-05.md` Q2 section.
...
> **⚠️ SYNC FROM `paper-draft.md` §6.1 (Pass-4 M4).** "Level-2 by construction" overstated as written; the audit-corrected form makes the conditions explicit. To sync: copy the current Theorem 6.1 from `paper-draft.md` §6.1 — it states (C1) **Positivity** (action mechanism $\pi_t(a \mid H_t) \ge p_{\min} > 0$ on the support of $H_t$), (C2) **Sequential ignorability / sufficient state** ($H_t$ blocks unobserved confounding), (C3) **Known action mechanism** ($\pi_t(a \mid H_t)$ is known) — and concludes that the loop generates samples from $P(o_{t+1} \mid \mathrm{do}(a_t), H_t)$ *conditional on the decision context*, not unconditional Level-2 data. Update §6 surrounding prose to invoke (C1)–(C3) consistently.
...
> **⚠️ SYNC FROM `paper-draft.md` §7.1 (Pass-4 + spike a5c98b95 N1).** Theorem 7.1 here is the pre-Pass-4 / pre-spike form. **Three substantive deltas to apply when syncing:**
>
> 1. **(A3) gains $p_{\mathrm{id}}$** (Pass-4 M3 / NC). The argmax-correctness probability $p_{\mathrm{id}} := \Pr[a^*_{\mathrm{ag},t} = \tilde a^*_t]$ is distinct from $\iota_{\mathrm{edge}}$ (related via DAG structure; equal in simple-bandit case). $K_t := D_{\mathrm{KL}}(\pi^*_t \,\|\, Q_t)$ defined locally.
> 2. **Conclusion (iii) bias bound** is now an expectation over misidentification mass: $\mathbb E[|\hat D_t - D_t^{\mathrm{true}}|] \le (1-p_{\mathrm{id}})\log(1/q_0)$, with $\hat D_t := -\log Q_t(a^*_{\mathrm{ag},t})$ and $D_t^{\mathrm{true}} := -\log Q_t(\tilde a^*_t)$. The visit-frequency $1/\sqrt n$ Hoeffding term is gone — $Q_t$ is internally available, no estimation-from-samples is needed at this layer. Full reframe in current Appendix F + Theorem F.1.
> 3. **(A5) → (A5') and (v) lifted to MDP trajectory level** (spike N1, 2026-05-05). (A5') is occupancy-weighted: $\sum_{t=1}^L \mathbb E[\overline{\mathrm{TV}}_t] \le 2c\sqrt L$ with $\overline{\mathrm{TV}}_t := (1/N_h)\sum_h \mathbb E_{s_h \sim d^{Q_t}_h}[\mathrm{TV}(\pi^*_t(\cdot|s_h), Q_t(\cdot|s_h))]$; UCRL2 / UCBVI satisfy natively. Conclusion (v) lifts to $\mathbb E[\mathrm{DynReg}(T)] \le 2cV_{\max}N_h\sqrt{(B_T+1)T} + N_h(1-p_{\mathrm{id}})\log(1/q_0)\cdot T$ via simulation lemma [Kakade–Langford 2002; Munos 2003; Ross–Bagnell 2010; Azar–Osband–Munos 2017]. The "not a full MDP trajectory-level theorem" scope clause is **removed**. Foster–Rakhlin DEC and TRPO bounds were considered and ruled out — see `_archive/spike-N1-N2-strengthen-2026-05-05.md`.
>
> Also: $B_T \to (B_T+1)$ off-by-one fix throughout (Pass-3 Codex Critical, applied in `paper-draft.md`); R-series rhetorical calibration ("strictly improves Pinsker/BH" → "lies strictly below the Pinsker/BH envelopes at this corner") throughout. To sync: copy current Theorem 7.1 statement + Appendix G(v) proof sketch verbatim from `paper-draft.md`.
...
> **⚠️ SYNC FROM `paper-draft.md` Appendix F (Pass-4 M3, 2026-05-05).** This appendix's framing as an *estimator* lemma is stale. The current `paper-draft.md` Appendix F is a **bias-bound lemma**: $Q_t$ is internally available (it's the agent's own policy distribution), so $\hat D_t := -\log Q_t(a^*_{\mathrm{ag},t})$ is computed directly — no visit-frequency estimator and no Hoeffding $1/\sqrt n$ variance term. The bias-from-misidentification piece survives and is formalized as Theorem F.1: $|\hat D_t - D^{\mathrm{true}}_t| \le \mathbf 1[a^*_{\mathrm{ag},t} \ne \tilde a^*_t]\cdot\log(1/q_0)$, expectation form $\mathbb E[|\hat D_t - D^{\mathrm{true}}_t|] \le (1-p_{\mathrm{id}})\log(1/q_0)$, with $p_{\mathrm{id}}$ disambiguated from $\iota_{\mathrm{edge}}$. Spike N1 added a per-state per-horizon-step lift remark used by Theorem 7.1(v): aggregating linearly over horizon gives the trajectory-level bias bound $N_h(1-p_{\mathrm{id}})\log(1/q_0)$ per round. To sync: replace this section verbatim from current `paper-draft.md` Appendix F. The estimation-from-samples concentration argument can be added as a remark for the case where $Q_t$ is unavailable internally and must be estimated from rollouts — but it's a separate concentration lemma, not the central bias-bound result.
