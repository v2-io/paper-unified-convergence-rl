...
*Cumulative rate: piecewise-stationary $B_T$ family by direct aggregation, continuous-variation $V_T$ family by MASTER wrapping.* The lifted UCRL2/UCBVI rate $\tilde O(N_h^2\sqrt{SA(B_T+1)T})$ ([[#^sec-algorithm]]) sits in the restart-on-change family of \cite{cheung-simchi-levi-zhu-2022-blessing}; via \cite{wei-luo-2021-blackbox}'s MASTER black-box, the same identity-routed base learner achieves Best-of-Both-Worlds $\tilde O(\min\{\sqrt{(B_T+1) T},\,(V_T+1)^{1/3}T^{2/3}\})$ automatically — matching \cite{mao-2021-nearoptimal}'s near-optimal $V_T$ exponent (parameter ($B_T$ vs $V_T$), $T$-scaling, $S,A$-scaling differ from Mao's RestartQ-UCB; full development in [[#^sec-conclusion]] + (A5')-BoBW Remark in [[#^sec-proof-composition]]).
